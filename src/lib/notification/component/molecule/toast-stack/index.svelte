...
{#if state.toasts.length > 0}
	<div class={state.containerClasses} {...props}>
		{#each state.toasts.slice(0, state.maxToasts) as toast}
			<div
				class={`w-full max-w-xs rounded-lg border p-4 shadow-lg ${state.getToastColor(toast.type)} ${props.toastClass ?? ''}`}
			>
				<div class="flex">
					<div class="flex-shrink-0">
						<Icon
							name={state.getToastIcon(toast.type)}
							class={`h-5 w-5 ${state.getToastIconColor(toast.type)}`}
						/>
					</div>
						...
						{#if toast.actions && toast.actions.length > 0}
							<div class="mt-3 flex space-x-2">
								{#each toast.actions as action}
									<button
										type="button"
										class="inline-flex items-center rounded border border-transparent bg-[var(--color-primary-100)] px-2.5 py-1 text-xs font-medium text-[var(--color-primary-700)] shadow-sm hover:bg-[var(--color-primary-200)] focus:outline-none"
