...
const X = 'x';
	...
	const alertClasses = $derived(
		joinClassNames('alert-container', `variant-${variant}`, className)
	);
	...
	const closeButtonClasses = $derived(joinClassNames('alert-icon', `variant-${variant}`));
		...
		<div class={mainContainerClasses}>
			{#if icon}
				<div class={iconContainerClasses}>
					<BaseIcon name={IconComponent} class={iconClasses} aria-hidden="true" />
				</div>
			{/if}
			...
			{#if closable}
				<div class={closeButtonContainerClasses}>
					<button
						...
						onclick={handleClose}
					>
						<BaseIcon name={X} class="h-5 w-5" aria-hidden="true" />
					</button>
				</div>
			{/if}
		</div>
