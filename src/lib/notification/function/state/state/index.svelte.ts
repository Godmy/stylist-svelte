...
export function createStateState(props: StateProps) {
	...
	const descriptionClasses = $derived(
		'mt-1 text-sm text-[var(--color-text-secondary)] max-w-md mx-auto'
	);
	const actionContainerClasses = $derived('mt-6');
	const actionButtonClasses = $derived(
		'inline-flex items-center px-4 py-2 border border-transparent shadow-sm text-sm font-medium rounded-md text-[var(--color-text-inverse)] bg-[var(--color-primary-600)] hover:bg-[var(--color-primary-700)] focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500'
	);
