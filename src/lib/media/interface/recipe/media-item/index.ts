/**
 * MediaItem — изображение / видео-превью / медиа-плитка.
 *
 * LEGO-состав:
 *   IMediaSlot        (information) — src, alt, thumbnail, srcSet, loading
 *   ILabelSlot        (information) — label (подпись под медиа)
 *   ICaptionSlot      (information) — caption, description
 *   IClickable        (interaction) — onClick (открыть в лайтбоксе)
 *   ISizable          (architecture) — size, shape
 *   IElevatable       (architecture) — layoutElevation (карточка с тенью)
 *   IMotionToken      (theme)       — duration, easing (fade-in при загрузке)
