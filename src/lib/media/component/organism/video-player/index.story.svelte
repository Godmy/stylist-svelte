...
<Story {id} {title} {description} component={VideoPlayer} category="Organisms" {controls}>
		...
		<section class="sb-organisms-video-player grid w-full gap-8 lg:grid-cols-[1fr_1fr]">
			...
			>
				...
				<p class="mt-1 text-[--color-text-primary]">
					Interactive video player with customizable options.
				</p>
			...
			>
				...
				<div class="mt-5 space-y-4">
					...
					>
						<p class="mb-2 text-sm font-semibold text-[--color-text-primary]">Autoplay Enabled</p>
						<div>
							<VideoPlayer
					...
					<article
						class="rounded-2xl border border-dashed border-[--color-border-primary] bg-[--color-background-primary] p-4"
					>
						<p class="mb-2 text-sm font-semibold text-[--color-text-primary]">No Controls</p>
						<div>
							<VideoPlayer
