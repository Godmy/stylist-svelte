...
export class LayoutStyleManager {
	...
	static resolveAspectRatio(props: { ratio?: number; class?: string }) {
		return {
			ratio: props.ratio ?? 1,
			classes: LayoutStyleManager.getAspectRatioClasses(props.class ?? '')
		};
	}

	static resolveContainer(props: { size?: TokenSize; class?: string }) {
		const size = props.size ?? 'full';
		return {
			size,
			classes: LayoutStyleManager.getContainerClasses(size, props.class ?? '')
		};
	}

	static resolveDivider(props: {
		...
		return {
			...
			align,
			dashed,
			baseLineClass: LayoutStyleManager.getDividerLineClass(dashed),
			leftLineFlex: LayoutStyleManager.getDividerLineFlexClass(align, 'left'),
			rightLineFlex: LayoutStyleManager.getDividerLineFlexClass(align, 'right'),
			isHorizontal: orientation === 'horizontal'
		};
	}

	static resolveGridLayout(props: {
		cols?: number;
		gap?: TokenSize;
		responsive?: boolean;
		alignItems?: TokenAlignment;
		...
		return {
			...
			justifyContent: props.justifyContent ?? 'justify',
			containerClass: LayoutStyleManager.getGridLayoutContainerClass(props.class ?? '')
		};
	}
	...
	static resolveSpacer(props: { axis?: TokenOrientation; size?: string | number; class?: string }) {
		const axis = props.axis ?? 'vertical';
		return {
			axis,
			sizeValue: LayoutStyleManager.getSpacerSize(props.size),
			classes: LayoutStyleManager.getSpacerClasses(axis, props.class ?? '')
		};
	}
}
