...
import { THEME_SCHEMES_BY_ID } from '$stylist/information/const/theme/scheme';
...
function toCSSCustomPropertySegment(value: string): string {
...
export function themeToCSSVars(theme: Theme): Record<string, string> {
	...
	stringEntries(theme.colors.graph.portDataType).forEach(([key, color]) => {
		vars[`color-graph-port-type-${toKebabCase(key)}`] = color;
	});

	stringEntries(theme.colors.graph.portState).forEach(([key, color]) => {
		vars[`color-graph-port-state-${toKebabCase(key)}`] = color;
	});
	...
	stringEntries(theme.size).forEach(([key, value]) => {
		vars[`size-${toCSSCustomPropertySegment(key)}`] = value;
	});
...
export function applyThemeModeAndScheme(
	...
	const theme = resolvedMode === 'dark' ? THEME_DARK : THEME_LIGHT;
