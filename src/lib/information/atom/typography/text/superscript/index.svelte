<script lang="ts">
	...
	import type { TypographyTextProps } from '$stylist/information/type/attribute/common';
	...
	const classes = $derived(
		`text-xs align-super ${disabled ? 'text-[var(--color-text-tertiary)]' : 'text-[var(--color-text-primary)]'} ${block ? 'block' : ''} ${props.class ?? ''}`.trim()
	);
</script>
