export class OperationsHistoryStyleManager {
	...
	static getOperationItemClasses(isSelected: boolean): string {
		const baseClasses =
			'p-3 hover:bg-[--color-background-hover] cursor-pointer transition-colors w-full text-left border border-transparent rounded hover:shadow-sm';
		const selectedClasses = isSelected
			? 'bg-[--color-primary-50] border-[--color-primary-200]'
			: '';
