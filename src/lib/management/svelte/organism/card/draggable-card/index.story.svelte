<script lang="ts">
  ...
  let status = $state('Waiting for interactionРІР‚В¦');
    ...
    <section class="sb-organisms-draggable-card grid w-full gap-8 lg:grid-cols-[1fr_1fr]">
      ...
      <div class="rounded-[2.5rem] border border-[--color-border-primary] bg-[--color-background-secondary] p-6 shadow-sm">
        ...
        <ul class="mt-4 space-y-2 text-xs text-[--color-text-secondary]">
          <li>РІР‚Сћ `onDragStart(data)` fired when dragging begins.</li>
          <li>РІР‚Сћ `onDragEnd(data)` fired after drop.</li>
          <li>РІР‚Сћ `onCardClick(data)` and `onContextMenu(data, event)` for extra affordances.</li>
        </ul>
      </div>
    </section>
  {/snippet}
