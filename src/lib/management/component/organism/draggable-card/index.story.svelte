	...
	{#snippet children(args)}
		<section class="sb-organisms-draggable-card grid w-full gap-8 lg:grid-cols-[1fr_1fr]">
			...
			>
				<header class="mb-4 flex items-center justify-between">
					<div>
						...
						<p class="text-lg text-[--color-text-primary]">
							Drag the card to trigger demos of drag callbacks.
						</p>
					</div>
					<span
						class="rounded-full bg-[--color-background-secondary] px-4 py-1 text-xs font-semibold text-[--color-text-primary]"
					>
				...
				<div
					class="h-[320px] rounded-2xl border border-dashed border-[--color-border-primary] bg-[--color-background-secondary] p-6"
				>
					<DraggableCard
						data={card}
			...
			>
				...
				<p class="text-sm text-[--color-text-secondary]">
					Use callbacks to update logs, analytics, or reorder lists.
				</p>
				<div
					class="mt-4 rounded-2xl border border-[--color-border-primary] bg-[--color-background-primary] px-4 py-3 text-sm"
				>
					{status}
				</div>
