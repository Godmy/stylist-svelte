...
const ArrowRight = 'arrow-right';
  ...
  let {
    ...
    description = 'Every component comes with ready-to-use code',
    code = '',
    ctaText = 'Try in Playground',
    ctaHref = '/playground',
    class: className = '',
    children,
    ...restProps
  ...
  <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
    ...
    <div class="max-w-4xl mx-auto bg-gray-800 rounded-2xl shadow-2xl overflow-hidden border border-gray-700">
      {#if children}
        {@render children()}
      {:else}
        <pre class="m-0 p-6 text-sm text-gray-100 overflow-x-auto"><code>{code}</code></pre>
      {/if}
    </div>
    ...
    <div class="text-center mt-12">
      ...
      >
        <BaseIcon name={Play} class="w-6 h-6" />
        {ctaText}
        <BaseIcon name={ArrowRight} class="w-6 h-6" />
      </a>
    </div>
  </div>
</section>
