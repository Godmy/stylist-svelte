import type { Props, FeatureCard, FeatureIcon } from '$stylist/development/type/struct/playground-feature-grid';
...
export function createPlaygroundFeatureGridState(props: Props) {
  const title = $derived(props.title ?? 'Why Stylist?');
  ...
  const features = $derived(props.features ?? [
    ...
    },
    {
      ...
      icon: 'git-branch' as FeatureIcon
    },
    {
  ...
  function getIcon(icon: FeatureIcon) {
    switch (icon) {
