...
export class FormStyleManager {
	...
	static label(base: string, required?: boolean, className?: string): string {
		const requiredClass = required
			? ' after:content-["*"] after:text-[--color-danger-500] after:ml-1'
			: '';
	...
	static input(base: string, error?: boolean, disabled?: boolean, className?: string): string {
		const errorClass = error
			? 'border-[--color-danger-500] focus:border-[--color-danger-500] focus:ring-[--color-danger-200]'
			: 'border-[--color-border-secondary] focus:border-[--color-primary-500] focus:ring-[--color-primary-200]';
		const disabledClass = disabled
			? 'bg-[--color-background-secondary] cursor-not-allowed opacity-[var(--opacity-60)]'
