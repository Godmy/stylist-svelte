...
export function createRadioGroupState(props: RadioGroupProps): SlotRadioGroupState {
	...
	const layoutClass = $derived(
		RADIO_GROUP_LAYOUT_CLASS[props.orientation ?? 'vertical'] ?? RADIO_GROUP_LAYOUT_CLASS.vertical
	);
	...
	const optionLabelClass = $derived(
		`flex items-start gap-3 rounded-md border border-[var(--color-border-primary)] px-3 py-2 transition-colors duration-[var(--duration-150)] ${props.disabled ? 'opacity-[var(--opacity-50)] cursor-not-allowed' : 'cursor-pointer hover:border-[var(--color-primary-400)]'}`
	);
	...
	const inputClass = $derived(
		'peer absolute h-4 w-4 rounded-full border border-[var(--color-border-primary)] text-[var(--color-primary-600)] focus:ring-indigo-500 focus:outline-none'
	);
	const indicatorClass = $derived(
		'pointer-events-none inline-flex h-2 w-2 rounded-full bg-[var(--color-primary-600)] opacity-[var(--opacity-0)] transition-opacity duration-[var(--duration-150)] peer-checked:opacity-[var(--opacity-100)]'
	);
