...
export function createValidationState(props: ValidationProps): SlotValidationState {
	const containerClasses = $derived(
		`field-group ${props.orientation === 'horizontal' ? 'flex items-start gap-3' : 'flex flex-col gap-2'} ${props.disabled ? 'opacity-[var(--opacity-70)] cursor-not-allowed' : ''} ${props.class ?? ''}`
	);
	const hasError = $derived(!!props.error);

	const rootClass = $derived('field-group');
	const labelClass = $derived(
		'field-label text-sm font-medium text-[var(--color-text-primary)] flex items-center gap-1'
	);
