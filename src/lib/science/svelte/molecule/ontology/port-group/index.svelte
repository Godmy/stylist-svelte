		...
		{#each ports as port, index}
			...
			<div class={`port-group__item ${divider && index > 0 ? 'port-group__item--divider' : ''}`}>
				<GraphPort
					...
					color={port.color}
					onclick={(e: MouseEvent) => handlePortClick(port, e)}
					onConnectionStart={(e: MouseEvent) => handleConnectionStart(port, e)}
					onConnectionEnd={(e: MouseEvent) => handleConnectionEnd(port, e)}
				/>
			</div>
		{/each}
