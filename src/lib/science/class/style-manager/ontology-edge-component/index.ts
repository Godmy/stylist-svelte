export class OntologyEdgeComponentStyleManager {
	static getEdgeStyle(relationshipType: string) {
		switch (relationshipType) {
			...
			case 'range':
				return {
					color: 'stroke-[--color-success-500]',
					style: 'stroke: 2',
					marker: 'url(#arrowhead-domain-range)'
				};
			case 'subPropertyOf':
				return {
					color: 'stroke-[--color-warning-500]',
					style: 'stroke-dasharray: 5, 5',
					marker: 'url(#arrowhead-subproperty)'
				};
			default:
				return {
					color: 'stroke-[--color-neutral-400]',
					style: 'stroke: 1',
