export class SentimentAnalysisStyleManager {
	...
	static getInputAreaClass(inputClass = ''): string {
		const baseClass =
			'w-full h-32 p-[--spacing-md] border border-[--color-border-primary] rounded-md shadow-sm focus:outline-none focus:ring-[--color-primary-500] focus:border-[--color-primary-500]';
	...
	static getAnalyzeButtonClass(isAnalyzing: boolean, isDisabled: boolean): string {
		const baseClass =
			'w-full inline-flex justify-center items-center px-[--spacing-md] py-[--spacing-sm] border border-transparent text-sm font-medium rounded-md shadow-sm text-[--color-text-inverse]';
		const enabledClass =
