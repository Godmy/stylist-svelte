...
export class ModelSelectorStyleManager {
	...
	static getDropdownButtonClass(dropdownClass: string = ''): string {
		const baseClass =
			'w-full flex items-center justify-between px-4 py-2 text-left bg-[--color-background-primary] border border-[--color-border-primary] rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-[--color-primary-500]';
	...
	static getChevronClass(isOpen: boolean): string {
		const baseClass =
			'h-5 w-5 text-[--color-text-secondary] ml-[--spacing-sm] transition-transform';
	...
	static getDropdownMenuClass(dropdownClass: string = ''): string {
		const baseClass =
			'absolute z-[var(--z-index-docked)] mt-1 w-full bg-[--color-background-primary] shadow-lg max-h-60 rounded-md py-1 text-base overflow-auto focus:outline-none sm:text-sm';
	...
	static getModelItemClass(isSelected: boolean, modelItemClass: string = ''): string {
		const baseClass =
			'flex items-start p-[--spacing-md] hover:bg-[--color-background-secondary] cursor-pointer';
