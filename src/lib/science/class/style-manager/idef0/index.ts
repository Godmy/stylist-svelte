...
export class Idef0StyleManager {
	...
	static getDiagramSvgClasses(className = ''): string {
		return mergeClassNames('c-idef0-diagram__svg block overflow-visible', className);
	}

	static getFunctionBoxGroupClasses(className = ''): string {
		return mergeClassNames('c-idef0-function-box', className);
	}
	...
	static getConnectorGroupClasses(className = ''): string {
		return mergeClassNames('c-idef0-connector', className);
	}

	static getConnectorLineClasses(className = ''): string {
		return mergeClassNames('c-idef0-connector__line stroke-[--color-text-secondary]', className);
	}
