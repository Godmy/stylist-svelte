...
export function createFriendListState(props: FRIEND_LIST_PROPS) {
	...
	function handleSearchInputEvent(event: Event): void {
		friendListHandleSearchInput(event, (value) => {
			searchQuery = value;
		});
	}
	...
	return {
		...
		get friendsByStatus(): Record<string, FRIEND_LIST_FRIEND[]> {
			return friendsByStatus;
		},
		handleSearchInputEvent,
		handleFriendClickEvent: (friend: FRIEND_LIST_FRIEND) =>
			friendListHandleFriendClick(friend, props.onFriendSelect),
		handleSendMessageEvent: (friend: FRIEND_LIST_FRIEND) =>
			friendListHandleSendMessage(friend, props.onSendMessage),
		handleCallEvent: (friend: FRIEND_LIST_FRIEND) => friendListHandleCall(friend, props.onCall),
		handleVideoCallEvent: (friend: FRIEND_LIST_FRIEND) =>
			friendListHandleVideoCall(friend, props.onVideoCall),
