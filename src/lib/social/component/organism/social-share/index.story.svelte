...
<Story {id} {title} {description} component={SocialShare} category="Organisms" {controls}>
		...
		<section class="sb-organisms-social-share grid w-full gap-8 lg:grid-cols-[1fr_1fr]">
			...
			>
				...
				<p class="mt-1 text-[--color-text-primary]">
					Interactive social share with customizable options.
				</p>
			...
			>
				...
				<div class="mt-5 space-y-4">
					...
					>
						<p class="mb-2 text-sm font-semibold text-[--color-text-primary]">Minimal</p>
						<div>
							<SocialShare
								{url}
								title={pageTitle}
					...
					<article
						class="rounded-2xl border border-dashed border-[--color-border-primary] bg-[--color-background-primary] p-4"
					>
						<p class="mb-2 text-sm font-semibold text-[--color-text-primary]">Full Featured</p>
						<div>
							<SocialShare
								{url}
								title={pageTitle}
