export class CommentThreadStyleManager {
	...
	static getComposerInputClass(inputClass = ''): string {
		const base =
			'w-full px-4 py-2 border border-[--color-border-primary] rounded-lg focus:outline-none focus:ring-2 focus:ring-[--color-primary-500]';
	...
	static getActionButtonClass(active = false): string {
		return active
			? 'flex items-center text-sm text-[--color-primary-600]'
			: 'flex items-center text-sm text-[--color-text-secondary] hover:text-[--color-text-primary]';
	}
}
