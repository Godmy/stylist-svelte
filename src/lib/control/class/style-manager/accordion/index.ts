...
export class AccordionStyleManager {
	...
	static getItemClass(itemClass: string = ''): string {
		const baseClass =
			'accordion-item border border-[--color-border-primary] rounded-lg mb-[--spacing-sm]';
	...
	static getHeaderClass(expanded: boolean, disabled: boolean, headerClass: string = ''): string {
		const baseClass =
			'w-full flex justify-between items-center p-[--spacing-md] text-left font-medium text-[--color-text-primary] hover:bg-[--color-background-secondary] rounded-lg';
	...
	static getContentPanelClass(expanded: boolean, contentClass: string = ''): string {
		...
		const sizeClass = expanded
			? 'max-h-96 opacity-[var(--opacity-100)]'
			: 'max-h-0 opacity-[var(--opacity-0)]';
