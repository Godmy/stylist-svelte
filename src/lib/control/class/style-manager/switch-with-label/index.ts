...
export class SwitchWithLabelStyleManager {
	...
	static getContainerClasses(labelPosition: LabelPosition, baseClass: string = ''): string {
		const positionClass =
			this.normalizePosition(labelPosition) === 'left' ? 'flex-row-reverse' : 'flex-row';

		return ['flex', 'items-center', positionClass, baseClass].filter(Boolean).join(' ');
	}
	...
	static getLabelClasses(disabled: boolean, labelClass: string = ''): string {
		...
		return ['text-sm', 'font-medium', 'text-[--color-text-primary]', disabledClass, labelClass]
			.filter(Boolean)
			.join(' ');
	}
