<script lang="ts">
  import { Icon as BaseIcon } from '$stylist'; const Columns2 = 'columns-2'; const Copy = 'copy'; const Eye = 'eye'; const GripVertical = 'grip-vertical'; const Image = 'image'; const Plus = 'plus'; const Save = 'save'; const Square = 'square'; const Trash2 = 'trash-2'; const Type = 'type'; const X = 'x';
import { PageBuilderStyleManager } from '$stylist/control/class/style-manager/page-builder';
import type { TokenComponentType as PageBuilderElementType } from '$stylist/architecture/type/enum/component-type';
  ...
  function getDefaultAttributes(type: PageBuilderElementType): PageBuilderElement['attributes'] {
    switch (type) {
      case 'text':
        return { FONT_SIZE: '16px', color: 'var(--color-text-primary)' } as any;
      case 'heading':
        return { level: 2, FONT_SIZE: '32px', color: 'var(--color-text-primary)' } as any;
      case 'button':
        return {
      ...
      case 'image':
        return { src: '', alt: 'Image', width: '100%', height: 'auto' } as any;
      case 'divider':
        return { color: 'var(--color-border-primary)' } as any;
      case 'container':
        return {
            ...
            <input
              ...
              oninput={(event) => {
                ...
                updateElement(selectedElement.id, {
                  attributes: { ...(selectedElement.attributes ?? {}), FONT_SIZE } as any
                });
              }}
            />
          </div>
