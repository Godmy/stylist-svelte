	...
	<div class="overflow-hidden rounded-lg border">
		<table class="min-w-full">
			<thead class="bg-[var(--color-background-secondary)]"
				><tr
					>{#each state.columns as c}<th class="px-3 py-2 text-left text-xs uppercase">{c}</th
						>{/each}</tr
				></thead
			>
			<tbody
				>{#each state.data as row, i}<tr class={state.getRowClass(i)}
						>{#each state.columns as c}<td class="border-t px-3 py-2 text-sm"
