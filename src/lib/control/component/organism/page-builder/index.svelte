...
<div class={PageBuilderStyleManager.getContainerClasses(state.className)} {...state.restProps}>
	...
	<div class="flex">
		<div class={PageBuilderStyleManager.getCanvasClasses(state.canvasClass, state.isPreviewMode)}>
			...
			{:else}
				<div
					class="mx-auto min-h-[400px] max-w-4xl rounded-lg border border-dashed border-[var(--color-border-primary)] p-4"
				>
					{#if state.elements.length === 0}
						<div
							class="flex h-64 flex-col items-center justify-center text-[var(--color-text-secondary)]"
						>
							<BaseIcon name={PAGE_BUILDER_ICON.SQUARE} class="mb-4 h-12 w-12" />
							<p>Drag elements here to start building your page</p>
							<p class="mt-2 text-sm">Or use the toolbar to add elements</p>
						</div>
					{:else}
						{#each state.elements as element, index}
							<div
								class={PageBuilderStyleManager.getElementClasses(
							...
							>
								...
								{:else if element.type === 'button'}
									...
									<button
										...
										class="cursor-pointer rounded border-0"
									>
										{element.content}
									</button>
								{:else if element.type === 'divider'}
									<hr class="my-2 border-t border-[var(--color-border-primary)]" />
								{:else if element.type === 'image'}
									{@const attrs = element.attributes as PageBuilderImageAttributes | undefined}
									<div
										class="flex h-32 items-center justify-center rounded border border-[var(--color-border-primary)] bg-[var(--color-background-secondary)]"
									>
										<BaseIcon
											name={PAGE_BUILDER_ICON.IMAGE}
											class="h-8 w-8 text-[var(--color-text-tertiary)]"
										/>
										<span class="ml-2 text-sm text-[var(--color-text-secondary)]"
											>Image: {attrs?.alt ?? 'Placeholder'}</span
										>
									</div>
								{/if}

								<div
									class="absolute top-0 -right-6 flex space-x-1 opacity-[var(--opacity-0)] group-hover:opacity-[var(--opacity-100)]"
								>
									<button
										type="button"
					...
					<div class="mt-4 flex justify-center">
						...
						>
							<BaseIcon name={PAGE_BUILDER_ICON.PLUS} class="mr-1 h-4 w-4" />
							Add Element
						</button>
					</div>
				</div>
		...
		{#if state.selectedElement && !state.isPreviewMode && state.editable}
			<div class={PageBuilderStyleManager.getPropertiesPanelClasses()}>
				<div class={PageBuilderStyleManager.getPropertiesPanelHeaderClasses()}>
					...
					<button
						type="button"
						class="rounded p-1 hover:bg-[var(--color-background-tertiary)]"
				...
				<div class="space-y-4">
					<div>
						<div class="mb-1 block text-sm font-medium text-[var(--color-text-primary)]">Type</div>
						<span class="text-sm text-[var(--color-text-primary)] capitalize"
