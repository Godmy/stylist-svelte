...
export function createZoomControlsState(props: ZoomControlsProps) {
	...
	const styles = {
		container:
			'inline-flex flex-col gap-3 rounded-lg border border-[--color-border-secondary] bg-[--color-background-primary] p-3 shadow-sm',
		indicatorContainer: 'flex justify-center',
		indicator:
			'inline-flex min-w-16 items-center justify-center rounded-md bg-[--color-background-secondary] px-3 py-2',
		controlsContainer: 'inline-flex items-stretch',
		controlButton:
			'inline-flex h-10 w-10 items-center justify-center border border-[--color-border-secondary] bg-[--color-background-primary] text-[--color-text-primary] transition-colors hover:bg-[--color-background-secondary]',
		controlButtonDisabled:
			'cursor-not-allowed opacity-[var(--opacity-50)] hover:bg-[--color-background-primary]',
	...
	const containerClasses = $derived(joinClassNames(styles.container, props.class));

	const controlsContainerClasses = $derived(
		joinClassNames(styles.controlsContainer, props.controlsClass)
	);

	const indicatorClasses = $derived(joinClassNames(styles.indicator, props.indicatorClass));
