...
export class EventCalendarStyleManager {
	...
	static getWrapperClasses(viewMode: string): string {
		const baseClasses =
			'event-calendar border border-[--color-border-primary] rounded-[--radius-lg] overflow-hidden bg-[--color-background-primary] shadow-[--shadow-sm]';
	...
	static getDateHeaderClasses(isToday: boolean): string {
		const baseClasses =
			'p-[--spacing-sm] font-[--font-weight-semibold] text-[--color-text-secondary]';
	...
	static getDayCellClasses(isToday: boolean, isCurrentMonth: boolean): string {
		const baseClasses =
			'min-h-[6rem] p-[--spacing-xs] border-t border-r border-[--color-border-tertiary] relative group';
		const todayClass = isToday ? 'bg-[--color-primary-50]' : '';
		const notCurrentMonthClass = !isCurrentMonth
	...
	static getEventClasses(hasColor: boolean, customColor?: string): string {
		const baseClasses =
			'text-[--text-size-xs] p-[--spacing-xs] rounded-[--radius-sm] truncate cursor-pointer text-[--color-text-primary] hover:opacity-[var(--opacity-90)] transition-opacity';
