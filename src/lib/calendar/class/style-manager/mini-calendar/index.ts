...
export class MiniCalendarStyleManager {
	...
	static getWrapperClasses(compact: boolean, hostClass: string): string {
		const baseClasses =
			'c-mini-calendar border border-[--color-border-primary] rounded-[--radius-lg] overflow-hidden';
	...
	static getDayCellClasses(
		...
		const baseClasses =
			'min-h-[--spacing-xl] p-[--spacing-xs] text-center border-t border-r border-[--color-border-tertiary] cursor-pointer';
