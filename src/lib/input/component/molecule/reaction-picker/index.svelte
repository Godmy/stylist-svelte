...
>
	...
	{#if state.showPicker && state.showEmojiPicker}
		<div
			class={`absolute bottom-full left-1/2 mb-2 -translate-x-1/2 rounded-lg border border-[var(--color-border-primary)] bg-[var(--color-background-primary)] shadow-lg ${state.pickerClass}`}
		>
			<div class="p-2">
				<div class="grid grid-cols-5 gap-2">
					{#each state.reactions as reaction}
						...
						<button
							type="button"
							class={`rounded-lg p-2 hover:bg-[var(--color-background-secondary)] ${
								reaction.active
	...
	<button
		type="button"
		class={`ml-1 rounded-full p-1.5 hover:bg-[var(--color-background-secondary)] ${
			state.hasActiveReaction
	...
	>
		{#if state.hasActiveReaction}
			...
			<BaseIcon
				...
				class={`h-5 w-5 ${state.firstActiveReaction ? 'fill-current' : ''}`}
			/>
		{:else}
