...
export class InputStyleManager {
	static getInputClasses(
		...
		return mergeClassNames(
			StyleManagerBase.classes.input,
			variantClass,
			sizeClass,
			errorClass,
			disabledClass,
	...
	static getInputBaseClass(): string {
		return StyleManagerBase.classes.input;
	}
	...
	static getInputErrorClass(): string {
		return INPUT_ERROR_CLASS;
	}
	static getInputSizeClass(size: TokenSize = 'md'): string {
		return INPUT_SIZE_CLASSES[size] ?? INPUT_SIZE_CLASSES.md;
	}
	static getTextareaSizeClass(size: TokenSize = 'md'): string {
		return TEXTAREA_SIZE_CLASSES[size] ?? TEXTAREA_SIZE_CLASSES.md;
	}
	...
	static getInputGroupInputClass(className = ''): string {
		return mergeClassNames(INPUT_GROUP_INPUT_CLASS, className);
	}
	static getInputGroupButtonClass(className = ''): string {
		return mergeClassNames(INPUT_GROUP_BUTTON_CLASS, className);
	}
	...
	static getPasswordToggleClass(className = ''): string {
		return mergeClassNames(INPUT_PASSWORD_TOGGLE_CLASS, className);
	}
	static getInputLongResizeHandleClass(className = ''): string {
		return mergeClassNames(INPUT_LONG_RESIZE_HANDLE_CLASS, className);
	}
	static getInputDoubleContainerClass(className = ''): string {
		return mergeClassNames(INPUT_DOUBLE_CONTAINER_CLASS, className);
	}
