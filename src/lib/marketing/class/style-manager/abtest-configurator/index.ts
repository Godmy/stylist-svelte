export class ABTestConfiguratorStyleManager {
	...
	static getInputClass(hasError: boolean = false): string {
		const baseClass =
			'block w-full rounded-md border-[--color-border-primary] shadow-sm focus:outline-none focus:ring-[--color-primary-500] focus:border-[--color-primary-500] sm:text-sm';
	...
	static getButtonClass(
		...
		const baseClass =
			'inline-flex items-center px-4 py-2 border border-transparent text-sm font-medium rounded-md shadow-sm text-[--color-text-inverse] focus:outline-none';
