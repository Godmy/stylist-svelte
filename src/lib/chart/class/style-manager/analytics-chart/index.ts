export class AnalyticsChartStyleManager {
	static getContainerClass(customClass: string = ''): string {
		const baseClass =
			'c-analytics-chart bg-[--color-background-primary] rounded-lg shadow border border-[--color-border-primary] overflow-hidden';
	...
	static getTrendContainerClass(trendValue: number): string {
		return `inline-flex items-center ${trendValue >= 0 ? 'text-[--color-success-500]' : 'text-[--color-danger-500]'}`;
	}
	...
	static getLegendContainerClass(legendClass: string = ''): string {
		const baseClass =
			'mt-[--spacing-md] grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5';
