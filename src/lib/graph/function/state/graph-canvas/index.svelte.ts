...
import type { GraphViewport } from '$stylist/graph/type/struct/graph-viewport';
import { DEFAULT_GRAPH_CANVAS } from '$stylist/graph/const/record/graph-canvas';
...
export function createGraphCanvasState(props: RecipeGraphCanvas) {
	...
	const viewport = $derived<GraphViewport>({
	...
	return {
		...
		get offset() {
			return { x: offsetX, y: offsetY } as GraphPosition2D;
		},
		get gridSize() {
			return gridSize;
		},
