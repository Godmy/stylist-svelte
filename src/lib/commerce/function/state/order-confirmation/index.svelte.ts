...
export function createOrderConfirmationState(props: Props) {
	...
	const formattedOrderDate = $derived(
		props.orderDate.toLocaleDateString(props.locale ?? 'en-US', {
			year: 'numeric',
			month: 'long',
			day: 'numeric'
		})
	...
	const formattedEstimatedDelivery = $derived(
	...
	const subtotal = $derived(
		props.items.reduce((sum: number, item: OrderItem) => sum + item.price * item.quantity, 0)
	);
