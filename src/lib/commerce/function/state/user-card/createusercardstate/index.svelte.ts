...
export function createUserCardState(props: UserCardProps) {
	...
	const rootClass = $derived(`user-card user-card-${size} ${props.class ?? ''}`.trim());
	...
	const avatarPlaceholderClass = $derived(
		'user-card__avatar-placeholder w-full h-full bg-[var(--color-background-tertiary)] rounded-full flex items-center justify-center'
	);
	const avatarInitialsClass = $derived(
		'user-card__avatar-initials text-lg font-semibold text-[var(--color-text-secondary)]'
	);
	const statusIndicatorClass = $derived(
		'user-card__status-indicator absolute bottom-0 right-0 w-3 h-3 bg-green-500 border-2 border-[var(--color-background-primary)] rounded-full'
	);
	const contentClass = $derived('user-card__content flex-1');
	const nameContainerClass = $derived(
		'user-card__name-container flex items-center justify-between'
	);
	...
	const moreButtonClass = $derived(
		'user-card__more-button p-1 hover:bg-[var(--color-background-secondary)] rounded'
	);
