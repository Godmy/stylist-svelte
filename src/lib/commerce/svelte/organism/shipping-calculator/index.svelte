<script lang="ts">
  ...
  import ShippingCalculatorComponent from './index.svelte';

  const ShippingCalculator = ShippingCalculatorComponent as any;
      ...
      <div class="rounded-[2rem] border border-[--color-border-primary] bg-[--color-background-primary] p-6 shadow-sm">
        ...
        <div class="mt-6">
          <ShippingCalculator
            ...
            onCalculate={(options: unknown[]) => console.log('Calculated shipping options:', options)}
          ...
          <article class="rounded-2xl border border-dashed border-[--color-border-primary] bg-[--color-background-primary] p-4">
            ...
            <div>
              <ShippingCalculator
                ...
                defaultWeight={1.2}
                onCalculate={(options: unknown[]) => console.log('Calculated options without dimensions:', options)}
                onOptionSelect={(option: typeof shippingOptions[number]) => console.log('Selected option:', option)}
              />
            </div>
          </article>
          ...
          <article class="rounded-2xl border border-dashed border-[--color-border-primary] bg-[--color-background-primary] p-4">
            ...
            <div>
              <ShippingCalculator
                ...
                onCalculate={(options: unknown[]) => console.log('Calculated options without weight:', options)}
                onOptionSelect={(option: typeof shippingOptions[number]) => console.log('Selected option:', option)}
              />
            </div>
          </article>
