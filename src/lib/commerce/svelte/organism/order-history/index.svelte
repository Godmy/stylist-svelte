<script lang="ts">
  ...
  import OrderHistoryComponent from './index.svelte';

  const OrderHistory = OrderHistoryComponent as any;
      ...
      <div class="rounded-[2rem] border border-[--color-border-primary] bg-[--color-background-primary] p-6 shadow-sm">
        ...
        <div class="mt-6">
          <OrderHistory
            ...
            onOrderClick={(order: typeof orderHistory[number]) => console.log(`Clicked order: ${order.orderNumber}`)}
            onRepeatOrder={(order: typeof orderHistory[number]) => console.log(`Reordering: ${order.orderNumber}`)}
            onDownloadInvoice={(order: typeof orderHistory[number]) => console.log(`Downloading invoice for: ${order.orderNumber}`)}
            onOrderRated={(orderId: string, rating: number) => console.log(`Rated order ${orderId} with ${rating} stars`)}
          />
        </div>
      </div>
