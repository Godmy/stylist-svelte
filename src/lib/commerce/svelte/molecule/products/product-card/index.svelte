<script lang="ts">
  ...
  let {
    ...
    variant?: 'detailed' | 'compact' | 'screen';
    title?: string;
    price?: number;
    currency?: string;
    image?: string;
  ...
  const titleClass = $derived(ProductCardStyleManager.getTitleClass(variant));
  ...
  const showActions = $derived((variant === 'detailed' || variant === 'screen') && actions);
