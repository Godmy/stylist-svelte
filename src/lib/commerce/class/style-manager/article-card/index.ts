...
export class ArticleCardStyleManager {
	...
	static getTitleClasses(className = ''): string {
		return cn('text-xl font-bold mb-2', className);
	}

	static getSubtitleClasses(className = ''): string {
		return cn('text-[var(--color-text-secondary)] mb-4', className);
	}
	...
	static getActionButtonClasses(className = ''): string {
		return cn('px-4 py-2 bg-[var(--color-primary-500)] text-[var(--color-text-inverse)] rounded hover:bg-[var(--color-primary-600)]', className);
	}
}
