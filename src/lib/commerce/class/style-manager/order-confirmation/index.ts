...
export class OrderConfirmationStyleManager {
	...
	static getContainerClasses(): string {
		return 'max-w-5xl mx-auto p-6';
	}
	static getHeaderClasses(headerClass: string = ''): string {
		return `mt-6 text-3xl font-semibold text-[--color-text-primary] ${headerClass}`.trim();
	}
	...
	static getOrderDetailsClasses(summaryClass: string = ''): string {
		return mergeClassNames('rounded-lg bg-[var(--color-background-secondary)] p-4', summaryClass);
	}
	static getShippingAddressClasses(addressClass: string = ''): string {
		return mergeClassNames(
			'rounded-lg border border-[var(--color-border-primary)] p-4',
			addressClass
		);
	}
	static getActionButtonsClasses(actionsClass: string = ''): string {
		return mergeClassNames('mt-8 flex flex-wrap gap-3', actionsClass);
	}

	static getOrderStatusClasses(
