/**
 * Менеджер стилей для компонента RecipeUpsell
...
export class UpsellStyleManager {
	...
	static getContainerClasses(additionalClass: string = ''): string {
		const baseClasses =
			'bg-[--color-background-primary] rounded-lg shadow border border-[--color-border-secondary]';
	...
	static getActionButtonClasses(variant: 'primary' | 'secondary'): string {
		const baseClasses =
			'w-full px-4 py-2 rounded-md text-sm font-medium focus:outline-none focus:ring-2 focus:ring-offset-2';
