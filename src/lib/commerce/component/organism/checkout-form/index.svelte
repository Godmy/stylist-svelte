						...
						<div class="flex items-center">
							<div
								class={`flex h-8 w-8 items-center justify-center rounded-full ${
									state.getStepStatus(step) === 'completed'
										? 'bg-[var(--color-primary-600)] text-[var(--color-text-inverse)]'
										: state.getStepStatus(step) === 'current'
											? 'border-2 border-[var(--color-primary-600)] bg-[var(--color-background-primary)] text-[var(--color-primary-600)]'
							...
							{#if i < 3}
								<div
									class={`flex-auto border-t-2 ${
										i < state.getStepIndex(state.currentStep)
	...
	<div class="grid grid-cols-1 gap-8 lg:grid-cols-3">
		...
		<div class="lg:col-span-2">
			...
			{#if state.currentStep === 'information'}
				<div class="rounded-lg bg-[var(--color-background-primary)] p-6 shadow">
					<h2 class="mb-6 flex items-center text-lg font-medium text-[var(--color-text-primary)]">
						<BaseIcon name={User} class="mr-2 h-5 w-5" />
						Contact Information
					</h2>
					...
					<div class="grid grid-cols-1 gap-6 sm:grid-cols-2">
						<div>
							<label
								for="firstName"
								class="mb-1 block text-sm font-medium text-[--color-text-primary]">First name</label
							>
							<input
								id="firstName"
								type="text"
								placeholder="John"
								class="block w-full rounded-md border border-[--color-border-primary] bg-[--color-background-primary] px-3 py-2 shadow-sm focus:border-[--color-primary-500] focus:ring-1 focus:ring-[--color-primary-500] focus:outline-none"
						...
						<div>
							<label
								for="lastName"
								class="mb-1 block text-sm font-medium text-[--color-text-primary]">Last name</label
							>
							<input
								id="lastName"
								type="text"
						...
						<div class="sm:col-span-2">
							<label for="email" class="mb-1 block text-sm font-medium text-[--color-text-primary]"
								>Email address</label
							>
							<input
								id="email"
						...
						<div class="sm:col-span-2">
							<label for="phone" class="mb-1 block text-sm font-medium text-[--color-text-primary]"
								>Phone number</label
							>
							<input
								id="phone"
					...
					<div class="mt-8">
						<h3 class="mb-4 flex items-center text-lg font-medium text-[var(--color-text-primary)]">
							<BaseIcon name={MapPin} class="mr-2 h-5 w-5" />
							Shipping Address
						</h3>
						...
						<div class="grid grid-cols-1 gap-4">
							...
							<div class="grid grid-cols-1 gap-4 pt-2 sm:grid-cols-2">
								<div class="sm:col-span-2">
									<label
										for="address1"
										class="mb-1 block text-sm font-medium text-[--color-text-primary]"
										>Address</label
									>
									<input
										id="address1"
									...
									{#if state.errors.address1}
										<p class="mt-1 text-sm text-[var(--color-danger-600)]">
								...
								<div>
									<label
										for="city"
										class="mb-1 block text-sm font-medium text-[--color-text-primary]">City</label
									>
									<input
										id="city"
										type="text"
			...
			{#if state.currentStep === 'shipping'}
				<div class="rounded-lg bg-[var(--color-background-primary)] p-6 shadow">
					<h2 class="mb-6 flex items-center text-lg font-medium text-[var(--color-text-primary)]">
						<BaseIcon name={Truck} class="mr-2 h-5 w-5" />
						Shipping Method
					</h2>
						...
						{#each state.shippingOptions as option}
							<div
								class={`flex items-start justify-between rounded-lg border p-4 ${
									state.selectedShippingOption === option.id
										? 'border-[var(--color-primary-500)] ring-2 ring-blue-500'
										: 'border-[var(--color-border-primary)]'
								}`}
							>
								<div>
									<div class="flex items-center">
										<input
											...
											value={option.id}
											class="h-4 w-4 border-[var(--color-border-primary)] text-[var(--color-primary-600)] focus:ring-blue-500"
										...
										<label
											...
											class="ml-3 block text-sm font-medium text-[var(--color-text-primary)]"
										>
											{option.name}
										</label>
									</div>
									<p class="ml-7 text-sm text-[var(--color-text-secondary)]">
										{option.description}
									</p>
								</div>
			...
			{#if state.currentStep === 'payment'}
				<div class="rounded-lg bg-[var(--color-background-primary)] p-6 shadow">
					<h2 class="mb-6 flex items-center text-lg font-medium text-[var(--color-text-primary)]">
						<BaseIcon name={CreditCard} class="mr-2 h-5 w-5" />
						Payment Method
					</h2>
			...
			{#if state.currentStep === 'review'}
				<div class="rounded-lg bg-[var(--color-background-primary)] p-6 shadow">
					<h2 class="mb-6 flex items-center text-lg font-medium text-[var(--color-text-primary)]">
						<BaseIcon name={Package} class="mr-2 h-5 w-5" />
						SlotReview Your SlotOrder
					</h2>

					<div class="space-y-6">
						<div>
							<h3 class="text-md mb-4 font-medium text-[var(--color-text-primary)]">
								Contact Information
							</h3>
			...
			{#if state.currentStep === 'confirmation'}
				<div class="rounded-lg bg-[var(--color-background-primary)] p-6 shadow">
					<div class="text-center">
						<div
							class="mx-auto flex h-12 w-12 items-center justify-center rounded-full bg-[var(--color-success-100)]"
						>
						...
						<p class="mt-2 text-[var(--color-text-secondary)]">
							Your order number is <span class="font-medium">var(--color-primary-700)</span>. We've
							emailed your receipt to {state.billingAddress.email}.
						</p>
						<div class="mt-6">
			...
			<div class="mt-8 flex flex-col sm:flex-row sm:justify-between sm:space-x-4">
				...
				<div class={state.currentStep === 'information' ? 'sm:col-span-2' : ''}>
					<Button variant="primary" class="w-full sm:w-auto" onclick={state.goToNextStep}>
		...
		<div class="sticky top-6 h-fit rounded-lg bg-[var(--color-background-primary)] p-6 shadow">
			<h2 class="mb-4 text-lg font-medium text-[var(--color-text-primary)]">SlotOrder Summary</h2>
					...
					{#each props.cartItems ?? [] as item}
						<li class="flex py-6">
							...
							<div class="ml-4 flex flex-1 flex-col">
								<div>
									<div
										class="flex justify-between text-base font-medium text-[var(--color-text-primary)]"
									>
										<h3>{item.name}</h3>
										<p class="ml-4">
											{state.formatCurrency(
			...
			<div class="mt-6 border-t border-[var(--color-border-primary)] pt-4">
				<div class="space-y-3">
					...
					<div
						class="flex justify-between border-t border-[var(--color-border-primary)] pt-3 text-base font-medium text-[var(--color-text-primary)]"
					>
			...
			{#if state.currentStep !== 'confirmation'}
				<div class="mt-6 space-y-2">
					...
					<p class="text-sm text-[var(--color-text-primary)]">
						Read our <a
							href="/terms"
							class="text-[var(--color-primary-600)] hover:text-[var(--color-primary-500)]"
