...
<div class={state.containerClasses}>
	<header class={state.headerClasses}>
		...
		<span class={state.getStatusBadgeClasses(state.subscription.status)}
	...
	{#if props.showBillingInfo !== false}
		<section class={state.billingInfoClasses}>
			<div class="flex items-center justify-between gap-4">
				...
				<span class={state.billingInfoValueClasses}
	...
	{#if props.showActions !== false}
		<div class={state.actionsContainerClasses}>
			<button
				type="button"
				class={state.getActionButtonClasses('primary')}
				onclick={props.onUpgrade}>Upgrade</button
			>
			<button
				type="button"
				class={state.getActionButtonClasses('secondary')}
				onclick={props.onPause}>Pause</button
			>
			<button type="button" class={state.getActionButtonClasses('danger')} onclick={props.onCancel}
				>Cancel</button
			>
		</div>
	{/if}
