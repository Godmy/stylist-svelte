...
<div
	class={mergeClassNames(BulkOrderFormStyleManager.getContainerClasses(hostClass), hostClass)}
	{...restProps}
>
	<div class={BulkOrderFormStyleManager.getHeaderClasses(headerClass)}>
		<div class="flex items-center">
			<BaseIcon name={Package} class="mr-2 h-6 w-6 text-[--color-primary-600]" />
	...
	<div class={BulkOrderFormStyleManager.getFormClasses(formClass)}>
		...
		{#if allowFileUpload}
			<div class={BulkOrderFormStyleManager.getFileAreaClasses()}>
				<Button variant="outline" class="mr-3">
					<BaseIcon name={Upload} class="mr-2 h-4 w-4" />
					Upload CSV
				</Button>
		...
		<div class={BulkOrderFormStyleManager.getItemsListClasses()}>
			...
			{#if state.items.length === 0}
				<div class={BulkOrderFormStyleManager.getEmptyStateClasses()}>
					<BaseIcon name={Package} class="mx-auto h-12 w-12 text-[--color-text-tertiary]" />
					<h3 class="mt-2 text-sm font-medium text-[--color-text-primary]">No items added</h3>
					<p class="mt-1 text-sm text-[--color-text-secondary]">
						Add some products to your bulk order.
					</p>
				</div>
			{:else}
				<div class="space-y-4">
						...
						{#if product}
							<div
								class={mergeClassNames(
									BulkOrderFormStyleManager.getItemContainerClasses(itemClass),
									itemClass
								)}
							>
								...
								<div class="ml-4 min-w-0 flex-1">
									...
									<div class={BulkOrderFormStyleManager.getQuantityAreaClasses()}>
										...
										>
											<BaseIcon name={Trash2} class="mr-1 h-4 w-4" />
											Remove
										</button>
									</div>
								</div>
		...
		<div
			class={mergeClassNames(
				BulkOrderFormStyleManager.getActionsAreaClasses(actionsClass),
				actionsClass
			)}
		>
