	...
	{#snippet children(values: any)}
		<div class="sb-organisms-rfqform space-y-6 p-6">
			...
			<div class="grid gap-6 xl:grid-cols-[2fr_1fr]">
				...
				<div
					class="space-y-5 rounded-lg border border-[var(--color-border-primary)] bg-[var(--color-background-primary)] p-4 shadow-sm"
				>
					...
					<div>
						<label
							for="rfq-description"
							class="block text-sm font-medium text-[var(--color-text-primary)]">Подзаголовок</label
						>
						<textarea
							id="rfq-description"
							rows="3"
					...
					<div class="grid gap-3 sm:grid-cols-2">
						<label class="flex items-center gap-2 text-sm text-[var(--color-text-primary)]">
							<input
								type="checkbox"
								bind:checked={showCompanyFields}
								class="rounded border-[var(--color-border-primary)]"
							/>
							Реквизиты компании
						</label>
						<label class="flex items-center gap-2 text-sm text-[var(--color-text-primary)]">
							<input
								type="checkbox"
								bind:checked={showShippingFields}
								class="rounded border-[var(--color-border-primary)]"
							/>
							Адрес доставки
						</label>
						...
						<label class="flex items-center gap-2 text-sm text-[var(--color-text-primary)]">
							<input
								type="checkbox"
								bind:checked={showDeliveryTerms}
								class="rounded border-[var(--color-border-primary)]"
							/>
							Условия поставки
						</label>
			...
			<div class="grid gap-4 md:grid-cols-2">
				...
				>
					...
					{:else}
						<p class="mt-2 text-sm text-[var(--color-text-secondary)]">
							Отправьте форму, чтобы увидеть живую статистику.
						</p>
					{/if}
				</div>
				...
				>
					...
					{:else}
						<p class="mt-2 text-sm text-[var(--color-text-secondary)]">
							События появятся после взаимодействия с формой.
						</p>
					{/if}
				</div>
			</div>
