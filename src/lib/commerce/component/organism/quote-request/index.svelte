...
<div class={InteractionInputStyleManager.root('c-quote-request', className)} {...restProps}>
	<form
		class={InteractionInputStyleManager.panel(`space-y-4 p-4 ${formClass}`)}
		onsubmit={(e) => {
			e.preventDefault();
			state.submit();
		}}
	>
		...
		<div class={`grid grid-cols-1 gap-3 md:grid-cols-2 ${sectionClass}`}>
			<input
				class={InteractionInputStyleManager.input(`px-3 py-2 ${inputClass}`)}
				placeholder="Name"
		...
		<div class={sectionClass}>
			<div class="mb-2 flex items-center justify-between">
				...
				<button
					...
					><BaseIcon name={Plus} class="mr-1 inline h-4 w-4" />Add</button
				>
			</div>
			<div class="space-y-2">
				{#each state.request.products as product, index}
					<div class="grid grid-cols-[1fr_100px_40px] gap-2">
						...
						/>
						<button type="button" class="rounded border" onclick={() => state.removeProduct(index)}
							><BaseIcon name={Trash2} class="mx-auto h-4 w-4" /></button
						>
					</div>
				{/each}
			</div>
		...
		{#if showShippingFields}
			<textarea
				class={InteractionInputStyleManager.input(`w-full px-3 py-2 ${inputClass}`)}
		...
		{#if showUrgentOption}
			<label class="flex items-center gap-2"
				><input type="checkbox" bind:checked={state.request.urgent} /> Urgent request</label
			>
		{/if}
		{#if showValidUntil}
			<input
				...
				oninput={(e) => state.setValidUntil((e.target as HTMLInputElement).value)}
			/>
		{/if}
		...
		<textarea
			class={InteractionInputStyleManager.input(`w-full px-3 py-2 ${inputClass}`)}
			placeholder="Message"
			bind:value={state.request.message}
		></textarea>

		<div class="flex justify-end gap-2">
			<button type="button" class="rounded border px-3 py-2" onclick={() => state.cancel()}
				>Cancel</button
			>
			<button
				type="submit"
				class={`rounded bg-[var(--color-primary-600)] px-3 py-2 text-[var(--color-text-inverse)] ${buttonClass}`}
				>Send request</button
			>
		</div>
	</form>
