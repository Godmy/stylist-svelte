...
<div class="c-domain-playground {className}">
	{#if screenState.currentScreen === DOMAIN_SCREEN.DOMAIN}
		{#await loadDomainExplorer() then module}
			...
			/>
		{/await}
	{:else if screenState.currentScreen === DOMAIN_SCREEN.WORKSPACE}
	...
	{:else if screenState.currentScreen === DOMAIN_SCREEN.BUILDER}
		{#await loadDomainBuilder() then module}
			{@const DomainBuilder = module.default}
			<DomainBuilder {tree} {descriptors} />
		{/await}
	{:else if screenState.currentScreen === DOMAIN_SCREEN.BACKLOG}
...
<style>
	...
	@media (max-width: 840px) {
		...
		.c-domain-playground:has(.workspace-screen) {
			padding-top: 0;
		}
