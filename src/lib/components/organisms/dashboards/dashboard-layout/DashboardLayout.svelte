...
<div class={`dashboard-layout flex h-screen bg-gray-50 ${hostClass}`} {...restProps}>
  <!-- Sidebar -->
  {#if sidebar}
    <aside 
      class={`hidden md:block ${sidebarWidthClass} bg-white shadow-md flex flex-col transition-all duration-300 ease-in-out ${sidebarClass}`}
    >
  ...
  <div class="flex-1 flex flex-col overflow-hidden">
    <!-- Header -->
    {#if header}
      <header class={`bg-white shadow-sm z-10 ${headerClass}`}>
    ...
    <!-- Footer -->
    {#if footer}
      <footer class={`bg-white border-t ${footerClass}`}>
