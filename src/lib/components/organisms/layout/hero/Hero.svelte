<script lang="ts">
  ...
  import AnimatedNumber from '../../../atoms/typography/animated-number/AnimatedNumber.svelte';
  ...
  const containerClasses = $derived(HeroStyleManager.getContainerClasses(height, className));
  const backgroundClasses = $derived(HeroStyleManager.getBackgroundClasses(backgroundVariant));
