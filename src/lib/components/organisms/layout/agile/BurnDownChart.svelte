...
<div bind:this={chartContainer} class="chart-container">
  <h3 class="chart-title text-lg font-semibold mb-3">{title}</h3>
  <svg width={width} height={height} class="border border-gray-300 rounded">
    <!-- Grid lines -->
    {#each Array(5) as _, i}
    ...
    <line 
      ...
      class="stroke-gray-700 stroke-2" 
    />
    <line 
      x1={margin.left} 
      y1={margin.top} 
    ...
    {#if data.points.length}
      <path 
        ...
        class="stroke-red-500 stroke-2 fill-none" 
      />
    {/if}
    ...
    {#each generateXLabels() as label}
      <text 
        x={label.x} 
        y={label.y} 
        class="text-xs fill-gray-700 text-anchor-middle" 
      >
        {label.text}
      </text>
    {/each}
    ...
    {#each generateYLabels() as label}
      <text 
        x={label.x} 
        y={label.y} 
        class="text-xs fill-gray-700 text-anchor-end" 
    ...
    <g transform={`translate(${width - 150}, ${margin.top})`}>
      <line x1="0" y1="0" x2="20" y2="0" class="stroke-blue-500 stroke-2" />
      <text x="25" y="5" class="text-xs fill-gray-700">Идеальный</text>
      <line x1="0" y1="20" x2="20" y2="20" class="stroke-red-500 stroke-2" />
      <text x="25" y="25" class="text-xs fill-gray-700">Фактический</text>
    </g>
  </svg>
</div>
