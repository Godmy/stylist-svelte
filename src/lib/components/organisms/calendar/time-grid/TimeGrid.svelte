<script lang="ts">
  import type { ITimeGridProps, DayTimeGrid, ExtendedTimeSlot } from './types';
  import type { CalendarEvent } from '$lib/components/molecules/controls/calendar/types';
  import type { ITimeSlot as TimeSlot } from '$lib/components/molecules/controls/calendar/time-slot/TimeSlot.types';
  import { TimeSlot as TimeSlotComponent, molecularTimeSlotToAtomic } from '$lib/components/molecules/controls/calendar/time-slot';
  import { TimeGridStyleManager } from './styles';
  ...
  const wrapperClasses = $derived(TimeGridStyleManager.getWrapperClasses(hostClass));
  const headerClasses = $derived(TimeGridStyleManager.getHeaderClasses(headerClass));
  ...
  {:else}
    ...
    <div class={timeGridClasses}>
      ...
      {#each timeGrid as day}
        <div class={TimeGridStyleManager.getDayColumnClasses(isWeekend(day.date))}>
          <div class={TimeGridStyleManager.getDayHeaderClasses(isToday(day.date), headerClass)}>
            ...
            {#each day.slots as slot, slotIndex}
              ...
              >
                <TimeSlotComponent
                  ...
                  events={slot.events}
                  class={TimeGridStyleManager.getTimeSlotClasses() + ' ' + slotClass}
                  onClick={() => handleSlotClick(day.date, slot.hour, slot.minute)}
                />
              </div>
              ...
              {#each slot.events as event}
                {#if event.allDay && showAllDayEvents}
                  <div
                    class={TimeGridStyleManager.getEventClasses(event.color, eventClass)}
