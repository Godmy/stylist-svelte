<script lang="ts">
  ...
  import { HeatmapStyleManager } from './styles';

  /**
   * Heatmap Component
   *
./Heatmap/typesмпонент Heatmap реализует следующие принципы SOLID:
   *
   * SRP (Single Responsibility Principle):
   * - Компонент отвечает только за отображение тепловой карты и обработку пользовательских взаимодействий.
   * - Логика стилизации вынесена в HeatmapStyleManager.
   *
   * OCP (Open/Closed Principle):
   * - Компонент открыт для расширения через пропсы (например, colorScheme, onCellClick).
   *
   * LSP (Liskov Substitution Principle):
   * - Компонент может быть заменен любым его наследником без нарушения корректности программы.
   *
   * ISP (Interface Segregation Principle):
   * - Интерфейс компонента определен через IHeatmapProps для четкого контракта.
   *
   * DIP (Dependency Inversion Principle):
   * - Зависит от абстракций (типов и интерфейсов), а не от конкретных реализаций.
   */
