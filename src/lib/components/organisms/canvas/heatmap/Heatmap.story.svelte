<script lang="ts">
  ...
  let title: string = 'Sales Performance by Month and Product';
  let width: number = 700;
  let height: number = 500;
  let showTooltip: boolean = true;
  let showLegend: boolean = true;
  let colorScheme: 'blue' | 'red' | 'green' | 'purple' | 'warm' | 'cool' = 'blue';
  let showAxis: boolean = true;
  let cellPadding: number = 2;
</script>
