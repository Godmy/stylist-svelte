<script lang="ts">
  ...
  import type {
    ...
    NotificationChannel,
    INotificationPreferencesCategory,
    INotificationPreferencesItem
  ...
  let {
    ...
    footerClass = '',
    ...restProps
  }: INotificationPreferencesProps = $props();

  let prefs = $state<INotificationPreferencesItem[]>([...preferences]);

  function togglePreference(categoryId: string, channel: NotificationChannel) {
    const prefIndex = prefs.findIndex((p: INotificationPreferencesItem) => p.categoryId === categoryId && p.channel === channel);
    ...
    if (onPreferenceChange) {
      onPreferenceChange(categoryId, channel, prefs.find((p: INotificationPreferencesItem) => p.categoryId === categoryId && p.channel === channel)?.enabled ?? false);
    }
  }
  ...
  function getPreference(categoryId: string, channel: NotificationChannel): boolean {
    const pref = prefs.find((p: INotificationPreferencesItem) => p.categoryId === categoryId && p.channel === channel);
    return pref?.enabled ?? true; // Default to true if not set
  }

  // Select all channels for a category
  function selectAllChannels(categoryId: string, select: boolean) {
    const category = categories.find((c: INotificationPreferencesCategory) => c.id === categoryId);
    if (!category) return;
  ...
  function allChannelsEnabled(categoryId: string): boolean {
    ...
    return category.channels.every((channel: NotificationChannel) => {
      const pref = prefs.find((p: INotificationPreferencesItem) => p.categoryId === categoryId && p.channel === channel);
