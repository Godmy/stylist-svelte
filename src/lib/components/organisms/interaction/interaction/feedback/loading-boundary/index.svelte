<script lang="ts">
  ...
  let { loading=false, error='', class: className='', ...restProps }: LoadingBoundaryProps = $props();
</script>
<div class={InteractionFeedbackStyleManager.root('c-loading-boundary border rounded-lg p-4', className)} {...restProps}>
  {#if loading}<div class="text-sm">Loading...</div>{:else if error}<div class="text-sm text-red-600">{error}</div>{:else}<slot>Ready</slot>{/if}
</div>
