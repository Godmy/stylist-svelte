<script lang="ts">
  ...
  function handleSort(columnKey: string) {
    const column = columns.find((col: Column) => col.key === columnKey);
  ...
  function getSortIcon(columnKey: string) {
    ...
    return sortConfig.direction === 'asc' 
      ? '▲'  // Using text arrows instead of icons for simplicity
      : '▼';
  }
...
<div class={`overflow-x-auto ${className}`} style={maxHeight !== 'none' ? `max-height: ${maxHeight}` : ''} {...restProps}>
  <table class={`min-w-full divide-y divide-gray-200 ${tableClass}`}>
        ...
        {#each columns as column}
          ...
          >
            <div class="flex items-center">
              ...
              {#if column.sortable}
                <span class="ml-1">{getSortIcon(column.key)}</span>
              {/if}
            </div>
          </th>
        {/each}
    ...
    <tbody class={`divide-y divide-gray-200 ${tbodyClass} ${maxHeight !== 'none' ? 'overflow-y-auto' : ''}`}>
