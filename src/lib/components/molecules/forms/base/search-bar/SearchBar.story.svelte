<script lang="ts">
  ...
  import SearchBar from '../../controls/search/search-bar/SearchBar.svelte';
  ...
  const filtered = $derived(() => {
    ...
    return destinations.filter((item) => item.toLowerCase().includes(lower));
  });
...
<Story
  ...
  component={SearchBar}
  category="Molecules"
  description="Debounced search input with clear and submit actions."
  ...
  {#snippet children(props: SearchBarStoryProps)}
    ...
    <div class="space-y-6">
      <SearchBar
        ...
        on:search={(event: CustomEvent<{ query: string }>) => handleSearch(event.detail.query)}
      />
