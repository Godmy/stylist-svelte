<script lang="ts">
  ...
  let startDateString = $state(test.startDate.toISOString().split('T')[0]);
  $effect(() => {
  ...
  let endDateString = $state(test.endDate ? test.endDate.toISOString().split('T')[0] : '');
  $effect(() => {
  ...
  function validateTest(): boolean {
    ...
    if (Math.abs(totalWeight - 100) > 0.1) { // Allow small floating point errors
      newErrors.weights = `Traffic distribution must total 100%. Current total: ${totalWeight}%`;
    }
  ...
  function addVariant() {
    ...
    // Calculate weight for the new variant
    let newWeight = newVariantWeight;
    if (newWeight <= 0) {
      // Distribute remaining weight equally among new variants
      const activeVariantsCount = test.variants.filter(v => v.isActive).length;
      const remainingWeight = 100 - test.variants.reduce((sum, v) => sum + v.weight, 0);
      newWeight = Math.round(remainingWeight / (activeVariantsCount + 1));
    }
    ...
    // Adjust other weights to accommodate the new one
    if (newWeight > 0) {
      const totalWeight = test.variants.reduce((sum, v) => sum + v.weight, 0) + newWeight;
      if (totalWeight > 100) {
        // Reduce other weights proportionally to keep total at 100%
        const excess = totalWeight - 100;
        test.variants = test.variants.map(v => ({
          ...v,
          weight: Math.max(0, v.weight - (v.weight / totalWeight) * excess)
        }));
      }
    }

    test.variants = [...test.variants, newVariant];
    newVariantName = '';
  ...
  function removeVariant(id: string) {
    test.variants = test.variants.filter(v => v.id !== id);
  }

  function updateVariantWeight(id: string, weight: number) {
    const variant = test.variants.find(v => v.id === id);
    if (variant) {
