<script lang="ts">
	...
	let nodes = $state<DemoNode[]>([
		{ id: 'node-1', label: 'Source', type: 'source', color: teal[500], position: { x: 80, y: 90 } },
		{ id: 'node-2', label: 'Gateway', type: 'gateway', color: amber[500], position: { x: 280, y: 50 } },
		{ id: 'node-3', label: 'Target', type: 'target', color: blue[500], position: { x: 500, y: 200 } }
	]);
	...
	function edgeStyle(from: DemoNode, to: DemoNode, values: any): Record<string, string> {
		...
		return {
			...
			'--edge-color': String(values.edgeColor ?? blue[600]),
				...
				{#each edges as edge (edge.id)}
					...
					{#if from && to}
						<GraphEdge
							...
							type={(values.edgeType as 'line' | 'curve' | 'polyline') ?? 'line'}
