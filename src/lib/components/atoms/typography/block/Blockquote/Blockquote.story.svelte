...
>
  {#snippet children(props: BlockquoteStoryProps)}
    <div class="space-y-8">
      <div class="rounded-2xl border border-gray-200/80 bg-white/70 p-6 shadow-sm dark:border-gray-800 dark:bg-gray-900/30">
        <Blockquote
          cite={props.cite}
        ...
        <p class="mt-4 text-sm text-gray-500 dark:text-gray-400">
          Toggle border/background to preview different emphasis treatments while keeping the same content.
        </p>
      </div>

      <div class="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
        {#each curatedQuotes as example}
          <div class="rounded-xl border border-gray-200/70 bg-white/60 p-4 shadow-sm dark:border-gray-800 dark:bg-gray-900/40">
            <Blockquote
              cite={example.cite}
