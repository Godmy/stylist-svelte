...
>
  {#snippet children(props: StatusIndicatorStoryProps)}
    <div class="space-y-8">
      <div class="flex items-center justify-between rounded-2xl border border-gray-200/80 bg-white/70 p-4 shadow-sm dark:border-gray-700/80 dark:bg-gray-900/40">
        <div>
          <p class="text-sm font-semibold text-gray-900 dark:text-white">Current selection</p>
          <p class="text-sm text-gray-500 dark:text-gray-400">Combine label overrides with semantic states.</p>
        </div>
        <StatusIndicator status={props.status} label={props.label} class="text-sm font-medium text-gray-800 dark:text-gray-100" />
      </div>
