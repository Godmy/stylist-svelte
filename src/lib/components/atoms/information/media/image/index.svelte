<script lang="ts">
  ...
  const imageClasses = $derived(`${baseClasses} ${loadedClass} ${transitionClass} ${sizeClasses} ${props.class ?? ''}`);

  const containerClass = 'relative inline-block overflow-clip bg-[var(--color-background-secondary,#f9fafb)] rounded-md';
  const wrapperClass = 'flex items-center justify-center w-full h-full min-h-[100px] text-[var(--color-text-secondary,#475569)]';
</script>
