  ...
  {#snippet children(props: { type?: 'single' | 'multiple'; disabled?: boolean })}
    <section class="grid w-full gap-8 lg:grid-cols-[1fr_1fr]">
      <div class="rounded-[2rem] border border-[--color-border-primary] bg-[--color-background-primary] p-6 shadow-sm">
        ...
        <p class="mt-1 text-[--color-text-primary]">Toggle group root that manages the state for its items.</p>
      ...
      <div class="rounded-[2rem] border border-[--color-border-primary] bg-[--color-background-secondary] p-6 shadow-sm">
        <h3 class="text-base font-semibold text-[--color-text-primary]">Toggle Group Root Variations</h3>
