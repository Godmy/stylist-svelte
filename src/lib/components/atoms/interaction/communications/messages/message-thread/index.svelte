              ...
              {#if !message.isOwn}
                <div class={getChatMessageHeaderClasses()}>
                  {#if message.senderAvatar}
                    <Avatar src={message.senderAvatar} alt={message.sender} size="sm" class="mr-2" />
                  {/if}
                  <span>{message.sender}</span>
                </div>
              {/if}

              <div class={getChatMessageBubbleClasses(message.isOwn, state.messageVariant ?? DEFAULT_CHAT_MESSAGE_VARIANT)}>
