...
export function createRadioState(props: RadioProps) {
  ...
  const radioWrapperClasses = $derived('');
  const radioClasses = $derived(
    `h-4 w-4 rounded-full border text-[--color-primary-500] focus:ring-[--color-primary-500] ${
  ...
  return {
    ...
    get radioWrapperClasses() {
      return radioWrapperClasses;
    },
    get radioClasses() {
      return radioClasses;
    },
