...
export const getTabClasses = (
  ...
  const sizeClass = size === 'sm' ? 'px-2 py-1 text-sm' : size === 'lg' ? 'px-4 py-2 text-base' : 'px-3 py-1.5 text-sm';
...
export const getTabPanelClasses = (isSelected: boolean, isDisabled: boolean, className = '') =>
  [isSelected ? 'block' : 'hidden', isDisabled ? 'opacity-50' : '', className].filter(Boolean).join(' ');
...
export const getTabIndicatorClasses = (color: string, isDisabled: boolean, className = '') => {
  ...
  return ['absolute bottom-0 h-0.5 transition-all', colorClass, isDisabled ? 'opacity-50' : '', className]
    .filter(Boolean)
    .join(' ');
};

export const getTabIndicatorStyle = (width: string, left: string) => `width: ${width}; left: ${left};`;
