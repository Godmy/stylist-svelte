...
<div class="space-y-8 p-6">
  <section class="rounded-2xl border border-[--color-border-primary] bg-white p-6 shadow-sm">
    <div class="grid gap-6 lg:grid-cols-[320px_1fr]">
      <div class="space-y-4">
        <div>
          <label for="label-input" class="text-sm font-medium text-[--color-text-secondary]">Label</label>
          <input
            id="label-input"
        ...
        <div>
          <label for="type-select" class="text-sm font-medium text-[--color-text-secondary]">Тип ввода</label>
          <select
            id="type-select"
            class="mt-1 w-full rounded-lg border border-[--color-border-primary] px-3 py-2 text-sm"
            bind:value={type}
