<script lang="ts">
	import type { GraphCanvasProps, GraphCanvasPosition } from '$stylist';
	import { createGraphCanvasState } from '$stylist/architecture/state/graph-canvas';
	import { DEFAULT_GRAPH_CANVAS } from '$stylist/information/const/default/graph-canvas';
	...
	function handleWheel(event: WheelEvent) {
		...
		const newZoom = Math.min(
			Math.max(props.minZoom ?? DEFAULT_GRAPH_CANVAS.minZoom, canvasState.zoom + delta),
			props.maxZoom ?? DEFAULT_GRAPH_CANVAS.maxZoom
		);
	...
	function snapToGrid(value: number): number {
		...
		const threshold = props.snapThreshold ?? DEFAULT_GRAPH_CANVAS.snapThreshold;
