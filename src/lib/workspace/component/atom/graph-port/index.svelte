<script lang="ts">
	...
	import createGraphPortState from '$stylist/workspace/function/state/graph-port/index.svelte';
...
<style>
	...
	.graph-port:hover .graph-port__hit-area {
		opacity: 0.1;
	...
	.graph-port[data-port-direction='input'] .graph-port__label { margin-left: 0.75rem; }
	.graph-port[data-port-direction='output'] .graph-port__label { margin-right: 0.75rem; }
	.graph-port__icon {
		display: inline-flex;
		align-items: center;
	...
	.graph-port--xs { --port-size: 8px; --hit-area-size: 16px; }
	.graph-port--sm { --port-size: 10px; --hit-area-size: 20px; }
	.graph-port--md { --port-size: 12px; --hit-area-size: 24px; }
	.graph-port--lg { --port-size: 16px; --hit-area-size: 32px; }
	.graph-port[data-port-direction='input'] { left: -6px; transform: translate(-100%, -50%); }
	.graph-port[data-port-direction='output'] { right: -6px; transform: translate(100%, -50%); }
	.graph-port[data-port-direction='input']:hover { transform: translate(-100%, -50%) scale(1.1); }
	.graph-port[data-port-direction='output']:hover { transform: translate(100%, -50%) scale(1.1); }
</style>
