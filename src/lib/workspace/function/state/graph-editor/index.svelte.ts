...
export function createGraphEditorState(props: GraphEditorProps) {
	...
	function handlePaletteNodeSelect(item: GraphEditorPaletteNode) {
		if (props.onNodeAdd) {
			...
			const editorWidth = editorRef?.clientWidth ?? 800;
			const editorHeight = editorRef?.clientHeight ?? 600;
			const centerX = (-offset.x + editorWidth / 2) / zoom;
			const centerY = (-offset.y + editorHeight / 2) / zoom;
