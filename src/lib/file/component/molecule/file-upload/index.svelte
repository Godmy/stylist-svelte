...
<div class={`file-upload-container ${state.className}`} {...state.restProps}>
	...
	{:else}
		<div
			class="file-upload-area rounded-lg border-2 border-dashed border-[var(--color-border-primary)] bg-[var(--color-background-secondary)] p-6 text-center"
			class:opacity-[var(--opacity-50)]={state.disabled}
		>
			<svg
			...
			<p class="mt-2 text-sm text-[var(--color-text-secondary)]">
				<label
					for="file-upload"
					class="relative cursor-pointer rounded-md bg-[var(--color-background-primary)] font-medium text-[var(--color-primary-600)] hover:text-[var(--color-primary-500)]"
		...
		{#if state.file}
			<div
				class="file-info mt-4 rounded-lg border border-[var(--color-border-primary)] bg-[var(--color-background-primary)] p-4"
			>
				...
				{#if state.status === 'uploading'}
					<div class="mt-4">
						<div class="h-2.5 w-full rounded-full bg-[var(--color-background-secondary)]">
							<div
								class="h-2.5 animate-pulse rounded-full bg-[var(--color-primary-600)]"
								style="width: 45%"
							></div>
						</div>
