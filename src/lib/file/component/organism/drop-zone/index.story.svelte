		...
		<section class="sb-organisms-drop-zone grid w-full gap-8 lg:grid-cols-[1.15fr_0.85fr]">
			...
			>
				...
				<p class="text-sm text-[--color-text-secondary]">
					As you drop files we mirror the payload below.
				</p>

				{#if droppedItems.length === 0}
					<div
						class="mt-6 rounded-2xl border border-dashed border-[--color-border-primary] bg-[--color-background-primary] p-4 text-sm text-[--color-text-secondary]"
					>
				...
				{:else}
					<ul class="mt-6 space-y-3">
						{#each droppedItems as item}
							<li
								class="rounded-2xl border border-[--color-border-primary] bg-[--color-background-primary] px-4 py-3 text-sm"
							>
