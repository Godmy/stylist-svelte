<script lang="ts">
	import { CLUSTER_TOOLBAR_ITEMS } from '$stylist/cluster/const/value/cluster-toolbar-items';
...
<nav
	class="c-cluster-toolbar {orientation} flex items-center gap-0.5 border-b border-[var(--color-border-primary)] bg-[var(--color-background-primary)] px-1 py-0.5 {className}"
>
	{#if showLabel}
		<div class="label">
	...
	{/if}
	{#each CLUSTER_TOOLBAR_ITEMS as item (item.name)}
		<button
			type="button"
			title={item.name}
