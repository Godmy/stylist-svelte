<script lang="ts">
	...
	import { createAudioSliderState } from '$stylist/audio/function/state/audio-slider/index.svelte';
...
<div class={`as-wrap ${state.className}`} {...state.restProps}>
	<div class="as-row">
		{#if state.label}
			<label for="audio-slider" class="as-label">{state.label}</label>
		{/if}
		<input
			id="audio-slider"
			type="range"
		...
		/>
		{#if state.showValue}
			<span class="as-value">{state.value}</span>
		{/if}
	</div>
</div>
