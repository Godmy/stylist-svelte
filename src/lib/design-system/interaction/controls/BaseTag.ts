...
export type BaseTagVariant = 'primary' | 'secondary' | 'success' | 'warning' | 'danger' | 'neutral';
export type BaseTagSize = 'sm' | 'md' | 'lg';

// Базовый интерфейс для компонентов тегов
export interface BaseTagProps<T = HTMLElement> extends HTMLAttributes<T> {
...
const SIZE_CLASSES: Record<BaseTagSize, string> = {
  sm: 'text-xs',
  md: 'text-sm',
  lg: 'text-base'
};
...
export const getBaseTagClasses = (
  baseClass: string,
  variant: BaseTagVariant = 'primary',
  size: BaseTagSize = 'md',
  ...
  return [
    ...
    SIZE_CLASSES[size],
    disabled ? STATE_CLASSES.disabled : 'cursor-default',
    className
  ]
    .filter(Boolean)
    .join(' ');
...
const BASE_CLOSE_BUTTON_CLASSES = 'ml-1 rounded-full focus:outline-none focus:ring-2 focus:ring-offset-2';
...
const CLOSE_BUTTON_SIZE_CLASSES: Record<BaseTagSize, string> = {
  sm: 'w-3 h-3 ml-1',
  md: 'w-3.5 h-3.5 ml-1',
  lg: 'w-4 h-4 ml-1'
};

export const getBaseTagCloseButtonClasses = (
  variant: BaseTagVariant,
  className = ''
) => {
  return [BASE_CLOSE_BUTTON_CLASSES, CLOSE_BUTTON_VARIANT_CLASSES[variant], className]
    .filter(Boolean)
    .join(' ');
};

export const getBaseTagCloseButtonIconClasses = (size: BaseTagSize) => {
