...
export interface StepConnectorStyleManagerInterface {
	...
	getStepIconClasses: (status: string, size: string) => string;
}
...
export const StepConnectorStyleManager: StepConnectorStyleManagerInterface = {
	...
	getStepContent: (step: { content?: string }) => step.content || '',
	getStepIconClasses: (status: string, size: string) =>
		cn(STEP_ICON_BASE_CLASS, `${STEP_STATUS_CLASS_PREFIX}${status}`, `${STEP_SIZE_CLASS_PREFIX}${size}`)
};
