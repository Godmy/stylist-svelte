...
const basePreset = <V extends string, S extends string>(variants: readonly V[], sizes: readonly S[], defaults: { variant: V; size: S }): Preset<V, S> => ({
  variants,
  sizes,
  ...
  classes: {
    ...
    size: Object.fromEntries(sizes.map((s) => [s, s === 'sm' ? 'h-8 px-3 text-xs' : s === 'lg' ? 'h-11 px-6 text-base' : 'h-9 px-4 text-sm'])) as Record<S, string>,
    variant: Object.fromEntries(variants.map((v) => [v, v === 'outline' ? 'border bg-transparent' : 'bg-primary text-white'])) as Record<V, string>,
    state: { disabled: 'opacity-50 cursor-not-allowed', block: 'w-full' }
  },
  loaderSize: Object.fromEntries(sizes.map((s) => [s, s === 'sm' ? 'w-3 h-3' : s === 'lg' ? 'w-5 h-5' : 'w-4 h-4'])) as Record<S, string>
});
...
export const CLOSE_BUTTON_PRESET = basePreset(BUTTON_VARIANTS, SIZES, { variant: 'ghost', size: 'sm' });
export const COPY_BUTTON_PRESET = basePreset(BUTTON_VARIANTS, SIZES, { variant: 'outline', size: 'sm' });
export const ICON_BUTTON_PRESET = basePreset(BUTTON_VARIANTS, SIZES, { variant: 'secondary', size: 'md' });
export const PAGE_BUTTON_PRESET = basePreset(BUTTON_VARIANTS, SIZES, { variant: 'outline', size: 'md' });
export const SPLIT_BUTTON_PRESET = basePreset(BUTTON_VARIANTS, SIZES, { variant: 'primary', size: 'md' });
