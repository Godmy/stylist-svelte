...
Иногда типы, определенные в общем файле (например, `$stylist/types/components.ts`), не соответствуют реальным использованным типам в компонентах. Также могут возникать конфликты имен между файлами компонентов и файлами типов, особенно с типами `GraphEdge`, `GraphNode` и другими.
