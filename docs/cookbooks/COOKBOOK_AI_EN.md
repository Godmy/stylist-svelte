...
```svelte
<button on:click={handleClick} on:change={handleChange}> Click me </button>

<input type="text" on:input={handleInput} on:focus={handleFocus} on:blur={handleBlur} />
```

**After:**

```svelte
<button onclick={handleClick} onchange={handleChange} role="button"> Click me </button>
...
Look for: Interfaces like `GraphEdge`, `GraphNode` in `$stylist/types/components.ts` that are only used in one or two components

### Fix Implementation

Move these types to component files locally:

**Before (in $stylist/types/components.ts):**
...
```svelte
<div onclick={handleClick} role="button" tabindex="0">Interactive element</div>
```
...
- `$stylist/components/atoms/index.ts` - updated exports as needed
- `$stylist/types/components.ts` - remove component-specific types
...
1. Replace imports like `import('$stylist/components/...').TypeName` with specific types
