...
async function run() {
	...
	for (const file of files) {
		...
		const tailwindPattern =
			/class="[^"]*(?:flex|grid|p-|m-|w-|h-|bg-|text-|border-|items-|justify-|relative|absolute|static|fixed|rounded|shadow|opacity-|z-|gap-|overflow-|flex-|grid-)[^"]*"/g;
		...
		const stateImportRegex = new RegExp(
			`from\\s+['"]\\$stylist/${domain}/function/state/${family}/index\\.svelte['"]`
		);

		if (!stateImportRegex.test(content)) {
			const stateFileTS = path.join(
				LIB_DIR,
				domain,
				'function',
				'state',
		...
		const interfaceImportRegex = new RegExp(
			`from\\s+['"]\\$stylist/${domain}/interface/recipe/${family}['"]`
		);
